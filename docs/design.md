...
| 2026-08-16 | A Chapter Was Emptied by a Base Captured Before It Loaded | The worst bug of the day, and not a streaming one: a chapter of prose was replaced with an empty document, auto-saved over, with no version snapshot behind it. The chat showed "已把进门选段再加厚…" followed by "1 suggested change could not be located… and was skipped" — the two halves of one failure. The rejoin captures `originalDocContent` as its leave-it-as-it-was base, and it ran before the chapter's LAZY content had loaded, so the base was ''. Every `<edit>` block then failed to match an empty document (hence the skip warning), and the completion path wrote the base back: `updateActiveDocument({ content: editDiffedDoc ?? originalDocContent })` with both sides empty. Two fixes, deliberately independent. (1) The cause: the rejoin awaits `ensureDocumentContents` before capturing the base — it already waited for the chat MESSAGE to arrive, but a message is not a document. (2) The backstop: `updateActiveDocument` refuses to blank a non-empty chapter at all. Blanking is a user action and arrives through the editor as its own HTML, never as ''; so the store can say no, and the next stale-base caller costs a console error instead of a chapter. Note what made it expensive rather than annoying: the rejoin path takes NO version snapshot, because snapshots are taken at send time and a rejoin does not send — the safety net was absent exactly where the bug lived. Both layers are mutation-checked: removing either turns its own test red. Media-only chapters count as content in the blank test, or an illustration-only chapter would have been "empty". |
| 2026-08-16 | Why One Paragraph Worked and Two Did Not | The user's observation — single-paragraph rewrites survive a reload, multi-paragraph ones lose both preview and diff — turned out to be exact and deterministic, not the model-output noise I had proposed. The probe measured the difference in one field: `Editor.tsx` publishes the selection as SERIALIZED HTML, and a selection inside one block comes out with no tags at all ("我推门。门轴轻轻一响", selBlocks 0) while one spanning two blocks comes out wrapped ("<p>一</p><p>二</p>", selBlocks 2). The haystack — the document flattened by `collectTextSpans` — has no tags, so the first matched by luck and the second could never match; relocation returned null, and everything downstream of it was skipped. Note how I got there wrongly first: I "reproduced" it with `selectNodeContents`, which wraps even a single paragraph, concluded BOTH cases returned null, and told the user my own fix could not be the explanation. The repro was not the app's selection. The user pushed back ("这真的是根本原因吗"), which was right to do, and the answer was that it IS the cause — reached only by measuring the real thing. Fix: reduce the needle the same way the haystack is reduced, with block tags collapsing to nothing rather than to a space (a separator would break the very case it fixes). Verified live: two-block selection, two reloads, three reattaches, 70 document updates during the stream, diff applied. |
| 2026-08-16 | Version History Had Been Silently Off for Seven Weeks | Investigated because an emptied chapter had nothing to roll back to. The server held 621 snapshots, none newer than 2026-06-28, across 52 books — and the cause was not one bug but three compounding. (1) NOTHING ever wrote a snapshot to the server: `POST /api/books/{id}/versions` existed, fully implemented, with zero callers — when the per-book API replaced the legacy whole-state `/api/storage` save in June, the write path was not carried over, and the endpoint's existence made it look done. (2) The sync then did `updates.versions = serverData.versions.map(...)` and wrote that to IndexedDB, so each reload REPLACED the local history with the server's empty list — history was not merely unsaved, it was actively deleted. (3) The 50-snapshot cap was global, so several books competed for the same 50 slots. A fourth defect surfaced while fixing: `restoreVersion` wrote `version.content` into the document, and a server-sourced snapshot carries `content: ''` until fetched — clicking Restore on any older snapshot would have blanked the chapter, the same shape as the bug that started this. Fixes: write through on create (and DELETE on delete, or the row returns at the next sync), merge instead of replace, cap per book via a `bookId` on the snapshot, backfill the local-only backlog after each sync, and fetch content before restoring — refusing outright if none can be had. The lesson worth keeping: an empty response from the authority is the absence of evidence, and this code read it as evidence of absence. Also note `tsc --noEmit` passed while `tsc -b` failed (composite project config) — always run the real build. |
| 2026-08-16 | Block-aware diff accept/reject | Rejecting a diff that ADDED a paragraph left a blank line behind: the resolution logic only ever deleted the marked *text*, so the `<p>` the diff had introduced survived as an empty block and the document never returned to its pre-diff bytes. Accepting a deletion had the mirror bug (`diffHtml` drops a deleted block's tags and emits its text as a bare `<del>`, which TipTap re-wraps in a paragraph of its own). Extracted the duplicated walk in `Editor.handleResolveDiff` and `useDiffHandlers` into `src/utils/diffResolution.ts`: `collectDiffRanges(doc, action, diffId?)` first finds textblocks whose entire visible content carries the mark this action erases and removes those blocks whole — expanding through ancestors the removal would leave empty (`li` inside `ul`), stopping short of emptying the document since `doc` requires `block+` — then collects the remaining text ranges as delete/unmark. `resolveDiffMarkupInHtml` gives the no-editor-mounted fallback the same shape. Tested against a real TipTap editor in `src/utils/__tests__/diffResolution.test.ts` (round-trip: original → `diffHtml` → reject → original, byte for byte). |
| 2026-08-17 | Cache-first context strategy (design) | Registered [cache_first_context.md](file:///home/hongdp/Workspace/web_canvas/docs/features/cache_first_context.md). Measured trigger: an ordinary chat turn against the local endpoint spent **42.87s before the first token**, all of it prefill of 4661 tokens, while the same engine replays an identical prefix in 0.2s. The 2026-07-07 layout already puts volatile content last, but the BULK lives there too — every attached reference chapter is rebuilt into the final user message each turn. Six prefix breakers identified with file references, two critical: the reference block sits in the volatile tail, and attached chapters are ordered by a score recomputed from each new prompt, so an unchanged chapter set is emitted in a different order turn over turn. Proposal: a session-scoped append-only **context ledger** injected as a `cacheHint` user/assistant pair before the history (generalizing `buildStickyBookPrefix`), insertion-ordered and never silently evicted; score decides admission, never position; the active document stays in the volatile tail because `<edit>` needs it adjacent to the request. Any eviction the user's request would force is confirmed through the existing inline whole-book consent panel with the real cost quoted. Acceptance is measurable without a model (assemble twice, compare prefixes) plus a 5× first-token drop on turn 2. Open questions carried in the spec: chapter-index placement, edited-chapter invalidation, whole-book path merge, and the 3-breakpoint cache_control cap. |
| 2026-08-17 | Removed Layer 2 (agentic chapter lookup) as dead code | Shipped in the smart-context feature, never used. The instruction reached the model on every multi-chapter turn (toggle default ON, `<lookup>` rules in the system prompt, `lookup_chapters` offered as a tool on the tool protocol), yet counting markers across the author's real book archive on the storage server found **0** `<lookup` requests and **0** `📖` status bubbles against **57** `Attached Context` labels — Layer 1's deterministic scorer was carrying the entire load. Removed in full: `parseLookupRequest`, `resolveLookupTitles`, `LookupLoopContext`, the `lookup_chapters` tool and its `allowLookup` gate, `CHAPTER_LOOKUP_RULES`, the `agenticLookupEnabled` setting and its Settings checkbox, and the continuation loop in `useChatLLM` (~27 edit sites). Two hazards the removal itself created and fixed: the deleted self-reference effect also served the **no-action retry** (leaving `startLLMStreamingRef.current` permanently null), and the retry call site still passed a positional `undefined` for the removed `lookupCtx`, shifting every argument after it. Also recorded: `npx tsc --noEmit` checks NOTHING in this repo (the root tsconfig only has references) — `tsc -b` is the real gate, and it is what caught both. The removed design and the questions worth asking before rebuilding it are kept in [smart_context_selection.md](file:///home/hongdp/Workspace/web_canvas/docs/features/smart_context_selection.md) §5. |
| 2026-08-17 | Cache-first context: the ledger, wired | Implemented [cache_first_context.md](file:///home/hongdp/Workspace/web_canvas/docs/features/cache_first_context.md) §4–§7. Reference chapters moved OUT of the final user message into an append-only block injected ahead of the history as a `cacheHint` user/assistant pair (the shape `buildStickyBookPrefix` already used), so a turn that changes nothing re-prefills only the active document and the request. `selectReferenceChapters` now takes `ledgerIds`: members ride free (no budget, no re-scoring) and the +30 continuity bonus applies only outside the ledger, where it can still change an outcome. Order is insertion order, permanently — the previous `[pinned, ...autos sorted by score]` re-emitted an unchanged chapter set in a different order every turn because the scores were recomputed from each new prompt. Removals keep the longest valid prefix and re-send only the suffix; the chapter that becomes active is always dropped rather than left as a stale duplicate, which is how an `<edit>` SEARCH silently stops matching. User-forced evictions raise an inline 3-option consent panel quoting the real re-read cost, defaulting to "keep it and continue". The ledger resets on book/provider/model change and whole-book mode. Chapter index moved to the volatile tail (it moves with every background summary refresh). Verified without a model: two turns assembled through the real hook, asserting the second request starts with the first and that the shared prefix carries the chapter text — mutation-checked. NOT done: history trimming from the front, rolling image stripping, and assembly-time same-role merging all still rewrite already-sent history (§3 breakers 3–5). |
| 2026-08-17 | History budgeted against the model's context window | Replaced the flat `MAX_HISTORY_CHARS = 80_000` with a per-turn budget: `contextTokens − maxOutputTokens − (system + ledger + active document) − 10% safety`, in the new `utils/contextWindow.ts`. The constant was wrong in both directions at once — the local endpoint runs 262144 tokens, so history was trimmed at roughly a tenth of what fits, while a 32k model would have been handed a prompt it must silently truncate from the FRONT, which is exactly the cached prefix this branch exists to protect. The window is DISCOVERED where possible: llama.cpp states `meta.n_ctx` on `/v1/models`, `/api/models` now forwards it as `contextWindows`, and `discoveredContextWindows` in the store overrides the shipped table — only the server knows what it was started with (this endpoint went 32K → 262144 in one day). Token estimation is CJK-aware: `length / 4` is an English rule that underestimates Chinese four-fold, so `estimateTokens` counts scripts separately and `tokensToChars` converts back at the conversation's measured CJK ratio. Editing `scripts/server_generation.py` requires restarting the API server. |
| 2026-08-17 | Prefix breakers 4 and 5 fixed; ledger ordered by stability | `trimHistoryForContext` now normalizes the whole history (drop empties, merge same-role runs, settle images) BEFORE cutting whole messages from the front, instead of cutting and then reshaping the survivors. Images no longer ride on history at all — the turn that attached one sends it, then the message keeps a fixed placeholder; the old rolling "last 4 keep theirs" rule silently changed a message's bytes as the conversation advanced past it. Merging before the cut makes a merged block atomic: cutting first let the window start INSIDE a same-role run and emit a fragment the model had never seen, ending the cached prefix there (mutation-checked). Separately, the ledger's admission order was rebuilt around the actual writing workflows: a permanently pinned outline is usually chapter zero, so the previous book-distance heuristic sorted it FIRST — the most expensive slot, since removing an entry re-sends everything after it — while it is the most frequently revised document in the book, and folding a new chapter back into it makes it the active document. `orderAdmissionsByStability` sorts by `updatedAt` (oldest first, book distance as tie-break), so finished chapters lead and the outline sinks to the end: that workflow now re-sends 500 characters instead of 18,000. Going back to finish an earlier chapter stays expensive once, then self-heals as its fresh timestamp sorts it last. Also registered the consolidated [context_engine.md](file:///home/hongdp/Workspace/web_canvas/docs/features/context_engine.md) design: per-provider cache profiles, with two hazards no layout can fix — grok's long-context price cliff counts CACHED tokens toward the threshold, and the local llama-server runs `--parallel 1`, so the background chapter summarizer evicts the conversation's KV prefix. |
| 2026-08-17 | Removed chapter-summary generation, kept the data | Summaries existed to feed Layer 0's chapter index and Layer 2's lookup. Layer 2 was deleted as dead code earlier today, and the author keeps a hand-written outline chapter that serves the index's purpose better because it is written deliberately. Deleted: `services/chapterSummaries.ts` (the summarizer, its queue and its status plumbing), the `summaryProvider` setting and its Settings control, `ProviderConfig.summaryModel`, the sidebar's summarize-all and per-chapter buttons plus the queue status strip, the overview drawer's summarize button and stale badge, and the generation-side helpers in `chapterIndex.ts` (`isSummaryStale`, `hashDocumentContent`, `detectSummaryLanguage`, `buildSummaryInput`, `MIN_CHARS_FOR_SUMMARY`) with their tests, plus 16 now-dead i18n keys per locale. KEPT deliberately, per the request: `summary`/`summaryContentHash` on `CanvasDocument`, their persistence and server sync, their display in the sidebar and overview, and their use by `buildChapterIndex` and the scorer's keyword-overlap signal — existing summaries keep working, new chapters fall back to a content digest. Side effect worth recording: this removes the last background producer of requests against the local endpoint, which runs `--parallel 1` — the summarizer was evicting the conversation's KV prefix between turns, which is the single-slot hazard filed in context_engine.md §3. |
| 2026-08-17 | Provider cache profiles, per-turn telemetry, and a live acceptance suite | Phase 1 of context_engine.md. `utils/providerProfile.ts` holds what each provider's cache actually does — automatic vs explicit, routing key, long-context price cliff, where cache hits are reported, and whether the cache is a single shared slot. The history budget now aims at `targetPromptTokens`, i.e. BELOW grok's 200K threshold rather than at the window, because that threshold counts cached tokens and doubles input, cached and output rates together. Telemetry: `lastTurnCache` in the store plus a footer readout, because session totals hide a single collapsed turn, and the backend's job-done line now carries the cache result next to the latency it explains. `scripts/acceptance/contextCache.accept.ts` (+ `vitest.acceptance.config.ts`, `npm run test:acceptance`) sends prompts built by the PRODUCT's own assembly to the live local model and reads `timings.cache_n`. Two measurement traps found and documented there: `timings.prompt_n` is what was COMPUTED, not the total (use `usage.prompt_tokens`), and llama.cpp keeps an LRU of past prompts in RAM (`--cache-ram`, 8 GiB default) — the first version reported a 99.9% hit for a prompt that differed at character 103 because an earlier run had already sent it, so every prompt now carries a per-run nonce. Result on the hybrid local model: continuing a conversation reuses 3690/4582 tokens and is 4.3× faster; an identical prompt is 140× faster; but appending a chapter inside the ledger reuses only 1510/4556, because a recurrent state cannot be rewound to an arbitrary token. `--swa-full`, denser context checkpoints and one-message-pair-per-chapter were each measured and changed nothing — recorded so they are not retried. The full win belongs to plain transformers, which is what the new telemetry will show on grok. |
| 2026-08-30 | Wake-on-request cloud endpoint | Registered [scripts/cloud_endpoint/README.md](file:///home/hongdp/Workspace/web_canvas/scripts/cloud_endpoint/README.md). A GPU VM that starts on the first request and stops after 15 minutes of quiet, so a bursty writing session pays for the GPU it uses instead of one that idles. `wake_proxy.py` is an OpenAI-compatible passthrough on loopback: it owns the machine's lifecycle, streams responses through unbuffered, and refuses to stop while a generation is in flight. It runs on the always-on workstation for two reasons — it must outlive the VM to start it, and the app's `/api/models` proxy rejects non-loopback hosts (SSRF guard), so fronting the cloud endpoint locally keeps the model listing working. Provisioning is deliberately ON-DEMAND rather than the mahjong project's DWS flex-start default: flex's termination action is DELETE so there is nothing to wake, and per that project's own cost doc a stopped flex VM restarted without quota "会立即失败（不像创建会排队）" — reliability the pattern depends on, at ~$4.5/h against $2.25/h. Two independent idle timers by design: the proxy's, and one on the VM itself, because the proxy runs on a machine that can crash and an abandoned GPU bills by the hour. Machine/model pairing: g4-standard-48 (RTX PRO 6000, 96GB Blackwell) with the NVFP4 build, which vLLM and SGLang support natively (SGLang day-0). Eleven pytest cases over the lifecycle, mutation-checked; the fake VM yields to the loop in every method because without that the concurrency test passes with the lock removed and proves nothing. |
| 2026-08-30 | Wake proxy: RunPod backend | Added `RunPodController` alongside the GCE one, and made it the default. Same class of card (RTX 6000 Pro, 96GB) at **$2.09/h against GCP's $4.5/h on-demand**, billed per second with no monthly minimum — a month with no writing costs nothing, which was the requirement. The lifecycle logic, its eleven tests and both idle timers were untouched: `VMController` existed for exactly this. Two properties of RunPod's API shape drove the code. `desiredStatus` is a DESIRE — it flips to RUNNING the moment a start is accepted, while the pod is still being placed — so readiness stays the health probe's job alone. And `portMappings` is per-run state: a pod that sleeps and wakes can return on a DIFFERENT public port, so an upstream URL read at startup is wrong from the first wake, the exact moment the proxy exists for. `upstream_url()` therefore re-reads it after every start and forgets it on stop, and `create_app` takes the address from the controller when it offers one. Nine new tests against a faked REST API, mutation-checked; the port-remap test stops the pod EXTERNALLY on purpose, because writing it the natural way (calling `stop()` first) passes even when `start()` forgets to drop the cached address — `stop()` clears it too, hiding the bug. |
| 2026-08-30 | Cloud endpoint: validated on real hardware, and serverless reversed | Rented an RTX PRO 6000 Server Edition for 46 minutes ($1.60) and measured the pairing before creating anything that bills monthly; pod deleted, no network volume made. Both open questions came back yes: the **full 262144 context fits** (59,727 MiB of 97,887 used) and prefill is **2540 tok/s against the workstation's 137 — 18.5x**, generation 110.5 vs 18.8 (5.9x). Output checked for coherence, not just rate. The reason it fits so easily is architectural and was not anticipated: `qwen4exp` carries a **3-gram-indexed per-layer embedding table** (`ple.ngram_size=3`, 16 heads x ~20M entries) that is the largest tensor in the file at **26.82 GiB**, and llama.cpp leaves it on the host mmap'd rather than in VRAM — so only 52.52 of the 79.34 GiB ever competes with the KV cache. That closes the accounting exactly (52.52 + 5.81 KV/compute = 58.33 GiB) and **puts a condition on the network-volume plan**: those 26.82 GiB are random per-token lookups out of an mmap'd file, cheap behind this pod's 1.5 TB page cache and not obviously cheap off network storage. **Serverless is now rejected**, reversing the previous entry's lean. Two measured reasons: it is **$3.49/h against the pod's $2.09** for the same card (the earlier table wrongly claimed billing parity), and scaling to zero **destroys llama.cpp's prefix cache** — measured at **0.25 s warm vs 11.78 s cold** on a 36k-token book-sized prefix, a 47x gap. A network volume persists files, not process memory, so a 5-second idle timeout discards that cache on every pause. Serverless is cheaper only in the configuration that throws the cache away; the configuration that keeps it bills wall-clock at 67% more than the pod. A warm pod, which is what the wake proxy manages, is the cheaper way to hold the cache. |
| 2026-08-30 | `runpod` provider: a config slot of its own | Added a sixth provider rather than reusing `ollama`. Both are OpenAI-compatible llama.cpp servers with no auth, so the code paths are shared — but one config slot cannot hold two endpoints, and trying the pod meant overwriting the local model's Base URL and typing it back afterwards. A separate slot is the whole feature. It defaults to the SSH tunnel (`http://127.0.0.1:8100/v1`) rather than a pod address, because RunPod remaps the public port on every start (see the wake-proxy entry above) while a tunnel's port does not. **Two places needed the provider, not the URL, to decide.** `stream_options` is rejected by llama.cpp and was detected by a loopback-or-keyless URL test; a pod addressed directly at `*.proxy.runpod.net` is neither, so the test now includes the provider in both `services/llm.ts` and `server_generation.py`. And `protocolChoice`'s NEEDS_TOOLS gained `runpod` — the same Qwen that never emits `<canvas>` is behind it, and defaulting to markup would have silently disabled document editing on the pod. **The `/api/models` SSRF guard was widened, narrowly.** It admitted loopback only, which an https `*.proxy.runpod.net` endpoint is not. `is_queryable_model_host` now also admits that one suffix over HTTPS only, matched as a leading-dot suffix so `evil.proxy.runpod.net.attacker.com` cannot satisfy it. Justified because the suffix resolves to RunPod's public proxy tier rather than anything on this network, the endpoint does one GET of `{base}/models` and returns parsed names, and reaching it already needs a session. No settings version bump: the migration's closing step already merges `DEFAULT_CONFIGS` over stored data, so adding the key upgrades existing envelopes — now asserted by a test rather than assumed. Nine tests added, all mutation-checked: substring-instead-of-suffix matching, dropping the HTTPS requirement, dropping the provider from the stream_options test, removing runpod from NEEDS_TOOLS, and merging over stored keys instead of default keys each turn exactly one test red. |
| 2026-08-30 | Cloud GPU verdict: quality loses to grok; pods deleted | The missing measurement was quality, and only real writing could produce it. A trial pod served all three viable quants (IQ3_XXS/IQ4_XS/Q4_K_M, the last at 85.9 of 96 GB — it fits) through the new provider tab, side by side with grok. User verdict: even Q4_K_M is far below grok, and at $79–174/month that ends the case — the pod's real wins (uncensored, 0.69 s cached first token vs grok's 45–199 s) are not worth the quality delta when the free local 27B covers the uncensored need. Pod deleted the same day (~$5 total for both experiments, zero recurring cost; a stopped-not-deleted pod would have billed its 400 GB disk ~$80/mo). The RunPod provider tab, validate_pod.sh and the measured numbers stay: they turn any future re-test — a better sub-90 GiB uncensored model shipping — into a ~$2, one-hour question. The local-model/grok split in local_model/README.md remains the operating answer. |
| 2026-08-30 | Edits apply against the same reading the model saw | User-reported: an <edit> block deleting a paragraph "没生效" — silently, every level of the matcher missing. The cause was a cleanliness asymmetry, not the matcher: dynamicContext deliberately shows the model `stripDiffMarkup(content)` (so SEARCH text is copied clean), but `applyEditBlocks` ran against the raw store content — which still carried the PREVIOUS turn's unresolved diff. A clean needle cannot match a haystack full of <ins>/<del> wrappers: exact/trimmed fail on the injected tags, the fuzzy pattern only forgives whitespace/quotes/entities, and the block-text fallback is poisoned by deletion-span text interleaving the comparable string. The exact user flow — rewrite a paragraph (diff pending), then ask to delete it — fails all five levels. Fix: the edits path and the <canvas> diff base both use `stripDiffMarkup(originalDocContent)` — the pending diff folds in as ACCEPTED, which is precisely what the model was told the document says, and the new turn lands as one fresh reviewable diff (diffing clean-vs-markup instead nests diff inside diff). Failure restores still return the untouched original, so a turn that applies nothing preserves the user's pending decisions. Two hook-flow tests build the pending state with the real diffHtml and drive the real hook; reverting either strip turns exactly its test red. |
| 2026-08-30 | Ledger eviction consent panel removed | The cache-first design's §6 gate — an inline panel blocking the send whenever removing a chapter would invalidate cached prefix — is gone, by user request: being interrupted about cache internals on a chapter removal was worse than the one-turn re-prefill it warned about. The removal now proceeds silently. `planLedgerTurn` still prices every drop (resendChars/resentIds, tests kept), so a future non-blocking indicator has the numbers; `planKeepingRemoved` and `requiresConsent` were deleted with the panel, along with the four i18n strings and the ChatPanel card. The whole-book consent (attaching an entire book) is a different gate and stays. cache_first_context.md §6 rewritten to record the reversal. |
| 2026-08-31 | Focus-time reload false positives: mutations return the stamp they wrote | User-reported: switching windows and returning sometimes threw the canvas back to the top of the chapter. The focus check compares the book's server `updated_at` against the last stamp the client recorded — but only the metadata PUT returned one (T1), while the document PUTs that follow each bump the book stamp further (T2) and returned bare `{success:true}`. After every sync containing a document change the baseline was therefore GUARANTEED stale, so the next focus event read T2≠T1, concluded another device wrote, and ran `switchBook` — a full reload that re-sets the editor (scroll lost) and cancels any pending debounced save (an unsaved edit within the 3s window silently discarded). An earlier fix removed the clock-drift false positives by comparing server-to-server; this removes the self-inflicted ones. All four document mutation endpoints (PUT, batch POST, DELETE, reorder) now return the `updatedAt` they wrote, and a store action `adoptServerUpdatedAt` records the MAX across responses (parallel PUTs resolve out of order; ISO-8601 UTC orders lexicographically) at all six client mutation sites. Real remote writes are still detected: another device's stamp is never returned to this client, so the comparison still trips. Two pytest cases assert each endpoint returns exactly the stamp the book row now holds; three vitest cases pin the max-adoption (adopting last-instead-of-max turns one red). |
| 2026-08-31 | Portrait scrolls the page, not a nested div (mobile Firefox) | User-reported: 7000-character chapters janked badly while scrolling on mobile Firefox; Chrome was smooth with the same content. Diagnosis was empirical, not deductive — a probe harness toggled candidate fixes by URL param (localStorage writes off, layerization, containment, read-only editor, no BubbleMenu, APZ hints) and all of them measurably failed or barely helped. Two PURE-STATIC control pages then settled it: identical text and typography, one scrolled by the page and one inside a nested overflow div — root smooth, nested janky. Gecko does not async-composite this subscroller, so no amount of CSS hinting fixes it and the structure has to change. In portrait the PAGE now scrolls: headers sticky, chat dock and drawers fixed. **Implementation note that matters more than the fix**: the canvas column is a fixed-height shell — six levels set height:100%/overflow:hidden INLINE so exactly one inner div scrolls. The first attempt overrode them from CSS with !important and leaked one layer at a time, each found only when the user hit a new visual bug (a 56px blank strip, a stranded toolbar, horizontal overflow). The shipped version instead makes the shell styles CONDITIONAL in JSX (`rootScroll` in App.tsx, a prop in Editor.tsx) so they are never emitted in portrait — there is no layer left to miss. Three CSS traps are documented inline because each cost a round: `overflow-x: hidden` next to `overflow-y: visible` computes the visible axis to `auto` and silently recreates a scroll container (use `clip`); `position: sticky` resolves against the nearest scroll container, so every stray one re-anchors the headers; and `position: fixed` escapes ALL ancestor clipping, so a closed drawer parked off-screen with translateX must also be `visibility: hidden`. A fourth was mine: `flex: none` on a child of a ROW container sizes it to max-content WIDTH, which let the toolbar's 740px button row drag every descendant past a 450px viewport. |
| 2026-09-05 | Stop keeps the draft; a preview must never reach the store by accident | User-reported: stopping a generation mid-stream rolled the editor back to the pre-stream document, but a reload showed the half-streamed draft — the screen and persistence disagreed. Two defects. (1) The leak: TipTap's `setEditable(editable, emitUpdate = true)` emits a synthetic `update` by default, and `Editor.tsx`'s `onUpdate` publishes `editor.getHTML()` to the store. The live `<canvas>` / `update_document` preview is written with `emitUpdate: false` precisely so it never reaches the store — but the Stop button flips `isStreaming`, the read-only effect called `setEditable(true)`, and the preview went into the store (then IndexedDB and the server) through that emit, milliseconds before the abort path reset the editor. The same emit fired on every editor mount and every stream start, writing the editor's normalized HTML back over the store each time. Fixed at the source: `setEditable(…, false)` — toggling read-only changes no content and must publish none. (2) The policy: the abort path discarded the draft on the premise that "the store was never updated", which was false (see 1) and, per the user, not wanted — a stopped draft should stay. Stop now commits the draft as ONE undo step (`keepCanvasPreview`): the pre-stream HTML is put back outside history, then the draft is applied as a single recorded transaction and written to the store, so Undo returns exactly the pre-stream document and the auto-snapshot taken before the send still holds it too. The bubble reports the stop instead of sitting on "Updating document...". Selection rewrites preview through real transactions and were already consistent; unchanged. Covered by hook flow tests (draft kept, store == editor, exactly one recorded write after an out-of-history restore, untouched document when stopped before any draft) and a component test that renders the real `Editor` against a real TipTap editor and asserts that flipping `isStreaming` around a preview publishes nothing — the mutation (dropping the `false`) turns it red. |
| 2026-09-06 | The server remembers each account's last active book | User-reported: logging in on a new device opened a book other than the one last edited. The book to open was chosen from localStorage alone (`web_canvas_active_book_id`, else `default`), so a fresh browser, a cleared cache, or an account switch (which clears the same keys) always landed in `default`; the server kept a per-book `active_document_id` but no per-user pointer to the book itself, and the book list's `updated_at` ordering was only used to fill the dropdown. Chosen over the client-only alternative ("open the most recently updated book when there is no local key") because the account, not the browser, owns "the book I was last editing". New SQLite table `user_state (username PK, last_active_book_id, updated_at)`, created by `init_db` on upgrade; `record_last_active_book` runs inside every book write (metadata PUT — which every debounced save and every book switch sends within seconds — plus create and the legacy storage save), and delete clears a matching pointer. `/api/auth/session` returns `lastActiveBookId`, resolved through a JOIN so it can never name a deleted book, falling back to the most recently updated book (so accounts predating the table get the right answer on their first login) and to null for an empty account. The client's init now prefers the server pointer over the local key, then `default`, and writes the choice back to the store and localStorage so later saves target the same book. Covered by five pytest cases (the pointer follows PUTs and beats recency, with the recency fallback before any write; a stale pointer after a deletion; create records; an empty account; a live-DB upgrade) and three vitest cases driving the real `initializeStoreFromServer` against a stubbed API. Method note: the first version of the PUT test passed with the record call removed — the PUT also bumps the book's `updated_at`, so the recency fallback alone satisfied it. Each test now bumps another book's stamp afterwards and asserts the pointer still wins; that is the assertion the mutation turns red. Note: `scripts/*.py` changed — the API server must be restarted for this to take effect. |
